    ...
    if (!Object.values(errors).some((error) => error)) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                alert(data.message || 'Something went wrong.'); // Provide more detailed feedback
                return;
            }
            ...
            setTimeout(() => {
                alert(`Your organisation is approved! Organisation ID: ORG12345, Password: ${formData.password}`);
                navigate('/organisation-page'); // Navigate to the organization page
            }, 3000);
        } catch (error) {
            console.error('Error submitting form:', error); // Log any errors encountered
    ...
    <div className="relative min-h-screen flex items-center justify-center bg-gray-100">
      ...
      >
        ...
        <div className="mb-4 relative flex items-center">
          ...
          />
          <button 
            type="button" // Add type to prevent default form submit behavior
            onClick={togglePasswordVisibility} 
            className="absolute right-3 text-gray-400"
          >
            {passwordVisible ? '🙈' : '👁️'}
